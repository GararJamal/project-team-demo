...
import { getSearchEntities, getEntities } from './role.reducer';
...
export const Role = (props: IRoleProps) => {
  ...
  return (
    <div>
      <h2 id="role-heading">
        <Translate contentKey="projectTeamDemoApp.role.home.title">Roles</Translate>
        <Link to={`${match.url}/new`} className="btn btn-primary float-right jh-create-entity" id="jh-create-entity">
          <FontAwesomeIcon icon="plus" />
          &nbsp;
          <Translate contentKey="projectTeamDemoApp.role.home.createLabel">Create new Role</Translate>
        </Link>
      </h2>
      <Row>
        <Col sm="12">
            ...
            <AvGroup>
              <InputGroup>
                <AvInput
                  ...
                  placeholder={translate('projectTeamDemoApp.role.home.search')}
                />
        ...
        {roleList && roleList.length > 0 ? (
            ...
            <tbody>
              {roleList.map((role, i) => (
                <tr key={`entity-${i}`}>
                  ...
                  <td>{role.cREATEDAT ? <TextFormat type="date" value={role.cREATEDAT} format={APP_DATE_FORMAT} /> : null}</td>
                  <td>{role.uPDATEDBY}</td>
                  <td>{role.uPDATEDAT ? <TextFormat type="date" value={role.uPDATEDAT} format={APP_DATE_FORMAT} /> : null}</td>
          ...
          !loading && (
            <div className="alert alert-warning">
              <Translate contentKey="projectTeamDemoApp.role.home.notFound">No Roles found</Translate>
            </div>
          )
        )}
      </div>
