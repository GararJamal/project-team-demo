...
export const getSearchEntities: ICrudSearchAction<IRole> = (query, page, size, sort) => ({
  type: ACTION_TYPES.SEARCH_ROLES,
  payload: axios.get<IRole>(`${apiSearchUrl}?query=${query}`),
});

export const getEntities: ICrudGetAllAction<IRole> = (page, size, sort) => ({
  type: ACTION_TYPES.FETCH_ROLE_LIST,
  payload: axios.get<IRole>(`${apiUrl}?cacheBuster=${new Date().getTime()}`),
});
