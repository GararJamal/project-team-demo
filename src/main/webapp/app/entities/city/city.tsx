...
export const City = (props: ICityProps) => {
  ...
  return (
    <div>
      <h2 id="city-heading">
        <Translate contentKey="projectTeamDemoApp.city.home.title">Cities</Translate>
        <Link to={`${match.url}/new`} className="btn btn-primary float-right jh-create-entity" id="jh-create-entity">
          <FontAwesomeIcon icon="plus" />
          &nbsp;
          <Translate contentKey="projectTeamDemoApp.city.home.createLabel">Create new City</Translate>
        </Link>
      </h2>
      <Row>
        <Col sm="12">
            ...
            <AvGroup>
              <InputGroup>
                <AvInput
                  ...
                  placeholder={translate('projectTeamDemoApp.city.home.search')}
                />
        ...
        {cityList && cityList.length > 0 ? (
            ...
            <tbody>
              {cityList.map((city, i) => (
                <tr key={`entity-${i}`}>
                  ...
                  <td>{city.nom}</td>
                  <td>{city.code}</td>
                  <td>{city.in ? <Link to={`country/${city.in.id}`}>{city.in.id}</Link> : ''}</td>
          ...
          !loading && (
            <div className="alert alert-warning">
              <Translate contentKey="projectTeamDemoApp.city.home.notFound">No Cities found</Translate>
            </div>
          )
        )}
      </div>
