...
export const ACTION_TYPES = {
  SEARCH_UTILISATEURS: 'utilisateur/SEARCH_UTILISATEURS',
...
export default (state: UtilisateurState = initialState, action): UtilisateurState => {
  switch (action.type) {
    case REQUEST(ACTION_TYPES.SEARCH_UTILISATEURS):
    ...
    case FAILURE(ACTION_TYPES.SEARCH_UTILISATEURS):
    ...
    case SUCCESS(ACTION_TYPES.SEARCH_UTILISATEURS):
...
const apiSearchUrl = 'api/_search/utilisateurs';

// Actions

export const getSearchEntities: ICrudSearchAction<IUtilisateur> = (query, page, size, sort) => ({
  type: ACTION_TYPES.SEARCH_UTILISATEURS,
  payload: axios.get<IUtilisateur>(`${apiSearchUrl}?query=${query}`),
});
