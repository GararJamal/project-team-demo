...
describe('Country e2e test', () => {
  ...
  it('should load create Country page', async () => {
    ...
    expect(await countryUpdatePage.getPageTitle().getAttribute('id')).to.match(/projectTeamDemoApp.country.home.createOrEditLabel/);
    await countryUpdatePage.cancel();
  });

  it('should create and save Countries', async () => {
    await countryComponentsPage.createButton.click();
    await countryUpdatePage.setNomInput('nom');
    expect(await countryUpdatePage.getNomInput()).to.match(/nom/);
    await countryUpdatePage.setCodeInput('code');
  ...
  it('should delete last Country', async () => {
    ...
    expect(await countryDeleteDialog.getDialogTitle().getAttribute('id')).to.match(/projectTeamDemoApp.country.delete.question/);
